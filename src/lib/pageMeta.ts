...
export const PAGE_META = {
    home: {
        title: '연봉계산기 | 연봉 실수령액·4대보험·퇴직금 계산기',
        description:
            '연봉 실수령액, 월급, 4대보험, 원천징수, 퇴직금, 주휴수당, 실업급여를 한 번에 계산할 수 있는 무료 연봉계산기입니다.',
        path: '/',
        keywords: ['연봉계산기', '실수령액 계산기', '월급 계산기', '4대보험 계산기'],
    },
    salaryCalculator: {
        title: '연봉 실수령액 계산기 | 월급·세후 급여 계산',
        description:
    ...
    annualLeavePayCalculator: {
        ...
        path: '/annual-leave-pay-calculator',
    ...
    weeklyHolidayPayCalculator: {
        ...
        path: '/weekly-holiday-pay-calculator',
    ...
    unemploymentBenefitCalculator: {
        ...
        path: '/unemployment-benefit-calculator',
...
export function buildMetadata(meta: PageMeta): Metadata {
    const url = meta.path === '/' ? SITE.url : `${SITE.url}${meta.path}`
